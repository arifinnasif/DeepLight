...
class Cal_params_epoch(object):
    ...
    def _transform_sum(self, y_true, y_pred, do_sigmoid=True):
        y_true = y_true.permute(1, 0, 2, 3, 4).contiguous()
        ...
        sum_true = torch.zeros(y_true[0].shape).to(y_true.device)
        sum_pred = torch.zeros(y_pred[0].shape).to(y_true.device)
    ...
    def cal_batch_neigh(self, y_true, y_pred, r, side, do_sigmoid=True, timesteps = 6):  # side:  side-length of one grid, km
        y_true, y_pred = self._transform(y_true, y_pred, do_sigmoid)
        y_pred = y_pred.reshape(-1,timesteps,159, 159)
        y_true = y_true.reshape(-1,timesteps,159, 159)
        
        y_true = F.max_pool2d(y_true, kernel_size=3, stride=1, padding=1)
        y_pred = F.max_pool2d(y_pred, kernel_size=3, stride=1, padding=1)
    ...
    def cal_batch_neigh2(self, y_true, y_pred, r, side, do_sigmoid=True, timesteps = 6):  # side:  side-length of one grid, km
        y_true, y_pred = self._transform(y_true, y_pred, do_sigmoid)
        y_pred = y_pred.reshape(-1,timesteps,159, 159)
        y_true = y_true.reshape(-1,timesteps,159, 159)
        # 0: unmarked, 1: hit, 2: miss, 3: false alarm, 4: correct negative
        y_sign = np.zeros((y_true.shape[0],timesteps,159, 159))
